...
def load_3_column_data(filepath):
    """
    Loads data from a 3-column space-separated file.

    The first two columns are converted to floats (x_data and y_data) and the third column kept as a string (annotations).
    ...
    try:
        ...
        return x_data, y_data, annotations
...
if dialog.ShowModal() == wx.ID_OK:
    ...
    if dlg.ShowModal() == wx.ID_OK:
        with open(selected) as d:
            ...
            if num_cols == 3:
                #load data
                x_data, y_data, annotations = load_3_column_data(selected)
                #regression
