...
if dialog.ShowModal() == wx.ID_OK:
    ...
    if df.shape[0]>6:
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            res.close()
            #Plot PCA results
            colormap = ['red', 'green', 'blue', 'cyan', 'magenta', 'yellow', 'black']
            labels = {
                str(i): f"PC {i+1} ({var:.1f}%)"
                for i, var in enumerate(pca.explained_variance_ratio_ * 100)
            }
            # --- Ensure z is a flat array of same length as X_reduced ---
            z = np.array(z).ravel()          # flatten safely
            assert len(z) == len(X_reduced), "z must have one value per sample"
            
            # --- Create DataFrame for Plotly ---
            df = pd.DataFrame(X_reduced, columns=[f"PC{i+1}" for i in range(X_reduced.shape[1])])
            
            # --- Decide coloring mode automatically ---
            if np.issubdtype(z.dtype, np.number) and len(np.unique(z)) > 10:
                # treat as continuous
                df["z"] = z
                fig = px.scatter_matrix(
                    df,
                    dimensions=[f"PC{i+1}" for i in range(X_reduced.shape[1])],
                    color="z",
                    labels=labels,
            ...
            fig.update_traces(marker=dict(size=5, opacity=0.7), diagonal_visible=False)
            fig.show()
